...
export const stripeSession = async data => {
  const { type, albumCover, dimensions, songName, artistName, size, frame, qrCode, shippingCharge } = data;
  ...
  if (type === 'custom') {
    ...
    const imgResponse = await fetch(`${process.env.BACKEND_URL}/custom?x=${x}&y=${y}&width=${width}&height=${height}`, {
      method: 'POST',
      body: formData
    });
